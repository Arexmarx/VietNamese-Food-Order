 ...
 * @author Admin
 */
public class AddFoodsToCartServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            Food food = fd.getFood(Integer.parseInt(foodId.trim()));
            if (cart == null) {
                cart = new HashMap<>();
                cart.put(food, 1);
            } else {
                Food find = null;
                for (Food tmp : cart.keySet()) {
                    if (tmp.getId() == Integer.parseInt(foodId.trim())) {
                        find = tmp;
                        break;
                    }
                }
            ...
            if (session.getAttribute("LoginObj") == null) {
                out.print("Hãy đăng nhập để có thể thêm vào giỏ hàng");
            } else {
