...
public class insertFoodServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            WeeklyMenu weekly = (WeeklyMenu) session.getAttribute("weeklyObj");
            String foodId = request.getParameter("foodId");
            int weeklyId = weekly.getId();
            WeeklyMenuItemDAO weeklyItemDao = new WeeklyMenuItemDAO();
            if (foodId != null && weeklyId != 0) {
                if (weeklyItemDao.insertFoodToWeeklyMenu(weeklyId, Integer.parseInt(foodId))) {
